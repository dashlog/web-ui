...
async function fetchAdditionalGithubData({ full_name, pulls_url, issues_url }) {
  ...
  const { data: pulls } = await httpie.get(pull, { headers: kGithubHttpHeaders });
  const { data: issues } = await httpie.get(issue, { headers: kGithubHttpHeaders });
...
async function fetchLastGithubCommit(commitsURL) {
  const uri = commitsURL.slice(0, commitsURL.length - kCommitUrlPostfixLen);

  const { data: commits } = await httpie.get(uri, { headers: kGithubHttpHeaders });
...
async function fetchGithubFile(name, fileName = "package.json") {
  ...
  try {
    const { data } = await httpie.get(uri, { headers: kGithubHttpHeaders });
...
async function fetchOrgInformation() {
  ...
  const { data } = await httpie.get(URI, {
    headers: kGithubHttpHeaders
  });
